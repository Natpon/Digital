...
architecture Structural of Elevator_Top is
    ...
    constant POS_F2 : STD_LOGIC_VECTOR(15 downto 0) := std_logic_vector(to_unsigned(620,  16));

    constant POS_F3 : STD_LOGIC_VECTOR(15 downto 0) := std_logic_vector(to_unsigned(1400, 16));
...
begin
    ...
    -- เรียกใช้งานโมดูล PWM_Generator ที่สร้างไว้

    Inst_PWM: entity work.PWM_Generator

        generic map(MAX_COUNT => 1000)
    ...
    -- Motor Driver (อัปเดตใหม่ให้ใช้สัญญาณ PWM)
    ...
    begin
        ...
        if rst = '1' then
            ...
            if sig_motor_run = '1' then
                ...
                ena_pwm <= sig_pwm_out; -- เปลี่ยนจาก '1' เป็นรับค่าจาก PWM Module
...
-- Position to Floor Converter
    ...
    begin
        ...
        case sig_current_floor is
            ...
            when "10"   => seg_out <= "1011011";

            when "11"   => seg_out <= "1001111";
